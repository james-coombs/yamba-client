  ...
  exports['default'] = _ember['default'].Component.extend({
    ...
    actions: {
      ...
      createPage: function createPage() {
        var page = new FormData(event.target); // Ember jQuery to find the form by it's id.
        this.sendAction('createPage', page); // Grabs the object which has been converted into FormData and sends it up to the route template
        // document.getElementById("new-page-form").reset();
        this.set('page', null); // Resets the Object
      }
    }
  });
});
...
define('yamba_client/uploads/service', ['exports', 'ember'], function (exports, _ember) {
  exports['default'] = _ember['default'].Service.extend({
    ...
    newPageUpload: function newPageUpload(page) {
      return this.get('ajax').post('/pages', { // This will use the ajax service and allow us to send a post request to the `/pages` route on in API
        data: page, // The FormData object which we have called `newPage`
        contentType: false,
        processData: false
      });
    }
